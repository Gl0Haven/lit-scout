...
先判定用户意图，选一个模式。**接需求方式见 `references/intake.md`（进门人设：同领域资深同门）**——
能从话里认出模式就别空问；只问会改变产出的 1–2 个关键缺口。**用户给的是模糊领域、没具体方向时，
不要空问"想做什么范围"，而是先轻量侦察该领域、把真实存在的子方向摆成带名字+一句话解释的菜单让他挑、
逐级收敛**（兼顾"就想广扫整个领域"与"想细做但不知道子领域名字"两种）。每个模式的详细流程在 references/ 下。
...
1. **圈定**：把模糊问题 → 可检索概念簇 + 同义词 + 关键作者/团队；与用户确认范围/年限/venue 偏好（按 `references/intake.md` 接需求；**领域模糊时先侦察、摆子方向菜单让用户逐级收敛**，而非空问范围）。记检索式/库/日期入 search-log（可复现）。
2. **扫**：派 `scout_agent`（多角度/多源，必要时并行），候选只来自工具返回。**覆盖领域核心、不偏科**：按时间分段（奠基/里程碑/近期 SOTA）+ 按引用中心度（OpenAlex cited_by 降序抓高被引核心，无论年份）两路并取，别只调研老文章或只调研新文章。
